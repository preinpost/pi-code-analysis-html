...
`make-html` 이 긴 글을 읽히는 *산문 리포트*(커버 → 섹션 → 콜아웃)라면, 이 스킬은 분석 결과를 **도해 중심으로 보여주는 페이지**를 만든다. 파이프라인 플로우 · 모듈 의존성 스택 · 스탯 타일 · CSS 바 차트 · 카탈로그 그리드 · 개선사항 카드를 따뜻한 에디토리얼 톤(아이보리 캔버스 + 클레이 액센트 + 세리프 디스플레이)으로 렌더한다.

**모든 컴포넌트는 일반화되어 있다** — 특정 코드베이스에 종속되지 않아 어떤 프로젝트 분석에도 재사용된다. 특히 아래 두 가지는 every analysis 에 **필수**로 들어간다.

- **프로그램 관여도 표** — *프로그램 = 파일 하나하나*. 모듈 행 아래 관련 파일 각각을 칩으로 표시하고 관여도(조회/쓰기/구성/설치)를 점으로 보여준다.
- **파일 의존성 그래프** — 관련된 **모든 파일**을 노드로 펼친 swimlane 의존성 그래프. 데이터 기반 vanilla-JS 템플릿(`LANES`/`NODES`/`EDGES` 세 배열만 채우면 레이아웃·엣지 라우팅·호버 하이라이트가 자동)으로 어떤 코드든 그릴 수 있다.
...
| 본문 | 산문 섹션 + 콜아웃 + 테이블 | 다이어그램 우선 섹션 + **프로그램 관여도 표 · 파일 의존성 그래프(필수)** |
